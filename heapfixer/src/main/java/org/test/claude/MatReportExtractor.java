...
public class MatReportExtractor {
    ...
    public MatReport extract(Path matReportsDir, String heapDumpPath) throws IOException {
        String leakText    = extractLeakSuspectsReport(matReportsDir);
        String overviewText = extractSystemOverview(matReportsDir);
    ...
    private String extractLeakSuspectsReport(Path reportsDir) throws IOException {
        // MAT writes a zip; look for it first, then fall back to a plain HTML file
        Path zip  = reportsDir.resolve("leak_suspects.zip");
        Path html = reportsDir.resolve("leak_suspects.html");

        if (Files.exists(zip)) {
            String content = readHtmlFromZip(zip, "leak_suspects.html");
            if (content != null) return stripHtml(content);
        }
        if (Files.exists(html)) {
            return stripHtml(Files.readString(html));
        }
        ...
        try (var stream = Files.walk(reportsDir, 2)) {
            Optional<Path> found = stream
                ...
                .findFirst();
            if (found.isPresent()) return stripHtml(Files.readString(found.get()));
        }

        return "[leak_suspects report not found in " + reportsDir + "]";
    }

    private String extractSystemOverview(Path reportsDir) throws IOException {
        // MAT puts it in system_overview/system_overview.html or a zip
        String[] candidates = {
            "system_overview/system_overview.html",
        ...
        for (String rel : candidates) {
            ...
            if (Files.exists(p)) return stripHtml(Files.readString(p));
        }
        Path zip = reportsDir.resolve("system_overview.zip");
        if (Files.exists(zip)) {
            String c = readHtmlFromZip(zip, "system_overview.html");
            if (c != null) return stripHtml(c);
        }
        return "[system_overview not found]";
    }
    ...
    private String extractDominatorTree(Path reportsDir) throws IOException {
        ...
        for (String rel : candidates) {
            Path p = reportsDir.resolve(rel);
            if (Files.exists(p)) {
                // Dominator tree HTML can be huge – cap at 8 KB to stay within token budget
    ...
    private String readHtmlFromZip(Path zipPath, String entryName) throws IOException {
        try (ZipFile zf = new ZipFile(zipPath.toFile())) {
            ...
            if (entry == null) {
                ...
                entry = zf.entries().asIterator().next();
            }
            if (entry != null) {
